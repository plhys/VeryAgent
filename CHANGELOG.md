...
- **团队状态感知（领班"知道自己在哪、在干嘛"）**：
  - **身份/状态注入**：`team_get_members` / `team_get_tasks` 响应现在带团队元信息（团队名 + 工作区），领班调用时能确认自己带的是哪个团队、在哪块工作区干活。
  - **状态序列化修复**：成员状态统一输出 `idle`/`working`/`stuck`/`error`（原为 `Idle`/`Working` Debug 格式）、任务状态输出 `pending`/`in_progress`/`completed`/`failed`（原带引号），领班读到的状态文本与前端一致、可直接理解。
  - **leader 角色提示升级（含老团队自动升级）**：新提示教领班——①每次老板来消息（哪怕只是打招呼）先调 `team_get_members` + `team_get_tasks` 刷新团队当前状态再回应，不凭创建时快照猜；②老板说「你好/你们团队怎么样/你能做什么」时：自我介绍 → 拉成员名单介绍团队 → 说明协作方式（目标→拆解→确认→派活→跟踪→汇报）→ 问老板想做什么；③布置任务时先查可用成员再拆解、派活前用 `ask_user_question` 确认、`team_assign_task` 派活、`team_get_tasks` 跟踪、完成后汇总。已存在团队在 `team_get` 时检测旧版提示并就地重新生成（幂等），无需重建团队。
- **团队协作逻辑闭环（老板 → 领班拆解 → 确认 → 自动派活 → 成员干活 → 结算汇总）**：
  - **后端派活引擎 `team_delegate_task`**（Tauri + Web 双通道）：一次调用完成「workspace→folder→建成员会话→写任务（slot 置 working）→spawn 成员智能体→以角色前缀+任务正文作首条消息下发」，返回新会话 id + 成员连接 id。
  - **任务自动结算**：成员会话 TurnComplete 时按 `end_turn`/异常自动将对应 `team_task` 结算为 completed（记录最终文本）或 failed，并发 `team://changed` 实时刷新任务板——不再需要手动改状态。
  - **团队 MCP 工具（领班自主拆解派活）**：veryagent-mcp companion 新增 `team` 功能组，3 个工具——`team_get_members`（列成员/角色/状态）、`team_assign_task`（自动派活，spawn 成员+发任务）、`team_get_tasks`（任务板状态+结果）。领班对话内自动注入，派活后 `team://changed` 刷新；`team_assign_task` 成功后在领班流上发 `team_member_started` 事件，前端按成员 key 以 viewer 挂载成员连接，成员小窗实时滚动其工作。
  - **leader 角色提示注入**：团队表新增 `leader_prompt` 列（迁移 `m20260812_000002_team_leader_prompt`），创建团队时生成中文 PM 角色设定（你是项目经理、用团队工具拆解派活、派活前用 ask_user_question 请老板确认、成员完成后汇总汇报），领班会话连接时注入。
