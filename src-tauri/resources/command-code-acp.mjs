    ...
    child.on("close", (code) => {
      ...
      for (const line of stdout.split("\n")) {
        ...
        if (
          ...
          trimmed.startsWith("──") ||
          trimmed.endsWith(":") ||
          trimmed.startsWith("cmdc") ||
          trimmed.startsWith("Docs:") ||
          trimmed.startsWith("Pass the")
        )
          continue
...
const initPromise = discoverModels()
  .then((models) => {
    cachedModels = models
...
function commandCodeConfigOptions() {
  const configuredModel =
    process.env.COMMAND_CODE_MODEL || "deepseek/deepseek-v4-flash"
...
function toolKindForName(name) {
  ...
  if (
    ...
    return "edit"
  if (n.includes("del") || n.includes("rm") || n.includes("remove"))
    return "delete"
...
async function handleRequest(msg) {
  ...
  switch (method) {
    ...
    case "session/new": {
      ...
      const models = cachedModels || []
      const defaultModel =
        models.length > 0 ? models[0].value : "deepseek/deepseek-v4-flash"
      session = {
        id: sessionId,
        cwd,
        model: defaultModel,
    ...
    case "session/set_config_option": {
      ...
      if (configId === "model") {
        ...
        (value === "auto" || value === "ask" || value === "yolo")
      ) {
        ...
        replyError(
          id,
          -32602,
          `Unsupported Command Code config: ${configId}=${value}`
        )
        return
      }
    ...
    case "tool_errored": {
      ...
      if (id && !deniedTools.has(id)) {
        emitToolCallUpdate(id, {
          status: "failed",
          rawOutput: {
            error: event.error?.message || event.error || "Tool error",
          },
        })
      }
      deniedTools.delete(id)
...
function handleResultFrame(result) {
  ...
  if (stop === "max_turns" || stop === "max_tokens")
