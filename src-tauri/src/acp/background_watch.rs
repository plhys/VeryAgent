...
//!
//! Claude Code produces activity outside any veryAgent-driven prompt turn:
...
//!   veryAgent-sent prompt turn are assembled into turns with the SAME Stage-A/
...
//!   by the **prompt ledger**: every prompt veryAgent sends is fingerprinted, and
...
/// Fingerprints of prompts veryAgent itself sent on this connection, so the
...
impl PromptLedger {
    ...
    /// Record the fingerprint of a prompt veryAgent is about to send: the first
...
/// not belonging to any veryAgent-sent prompt turn, assembled into turns via the
...
enum Mode {
    /// Records belong to a veryAgent-sent prompt turn — the wire renders them.
    Foreground,
    ...
    fn classify_and_feed(
        ...
        if let Some(initiator_text) = turn_initiator_text(value) {
            if ledger.consume_matching(&initiator_text) {
                // A veryAgent-sent prompt: the wire renders this turn. Close any
...
fn turn_initiator_text(value: &serde_json::Value) -> Option<String> {
    ...
    if let Some(s) = content.as_str() {
        ...
        // A slash command persists as command tags; veryAgent sent the display
...
mod tests {
    ...
    fn pre_discovery_records_are_accounted_and_ledger_consumed() {
        ...
        // On disk before discovery: veryAgent's first prompt, the reply, an ack.
        write_lines(
            &path,
            &[
        ...
        assert!(
            turns.is_empty(),
            "the veryAgent-sent prompt classifies foreground — the wire renders it"
        );
        ...
        write_lines(
            &path,
            &[
                &cron_prompt("do the thing"),
                &assistant_text("a2", "pass"),
            ],
        );
        let (turns, ..) = unpack(tick_now(&mut ws, &ledger).expect("turns event"));
        assert!(
    ...
    #[test]
    fn veryagent_sent_prompt_is_foreground_and_not_surfaced() {
        let dir = tempfile::tempdir().unwrap();
        let path = temp_session(&dir);
        write_lines(&path, &[]);
        let ledger = PromptLedger::shared();
    ...
    fn same_text_refire_without_ledger_entry_is_background() {
        // The /loop case: veryAgent sent the text once (consumed), the scheduler
