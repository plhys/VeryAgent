...
/// Global side-channel announcing a shared-identity (body layer) save. Same
/// cross-window rationale as [`FEEDBACK_SETTINGS_CHANGED_EVENT`]: the settings UI
/// runs in a separate window, so other clients learn the profile/sharing changed
/// only via this backend broadcast. Payload: `SharedIdentitySettings`.
pub const SHARED_IDENTITY_SETTINGS_CHANGED_EVENT: &str = "shared-identity-settings://changed";
