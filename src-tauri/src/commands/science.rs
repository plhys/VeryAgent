...
//! experts — `~/.veryagent/skills/<id>/`. Users enable a science skill for any ACP
...
// ─── Paths ──────────────────────────────────────────────────────────────
// The central store is shared with experts (`~/.veryagent/skills/`); only the
...
fn unlink_one_locked(skill_id: &str, agent_type: AgentType) -> Result<(), ScienceError> {
    ...
    for dir in dirs {
        ...
        if matches!(
            state,
            ExpertLinkState::LinkedToApp | ExpertLinkState::Broken
        ) {
...
mod tests {
    ...
    async fn apply_links_does_not_deadlock() {
        let ops = vec![
            LinkOp {
                expert_id: "zzz-veryagent-science-batch-absent-aaa".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
                expert_id: "zzz-veryagent-science-batch-absent-bbb".into(),
    ...
    async fn apply_links_collects_per_op_results_without_aborting() {
        let ops = vec![
            LinkOp {
                expert_id: "zzz-veryagent-science-batch-absent".into(),
                agent_type: AgentType::ClaudeCode,
                enable: false,
            },
            LinkOp {
