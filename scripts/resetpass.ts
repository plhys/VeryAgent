 ...
 *   VERYAGENT_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@veryagent/web-host';
 ...
 * See the comment there for why the default is `~/.veryagent-web*` (not `~/.aionui*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `VERYAGENT_PORT` to match
 ...
 * pass --port / VERYAGENT_PORT if they launched webui on a non-default one.
 */
