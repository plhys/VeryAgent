# Publish v0.9.5 installer + .sig + latest.json to GitHub plhys/VeryAgent
...
$Staging = Join-Path $Root ".release-upload-0.9.5"
$Repo = "plhys/VeryAgent"
$Tag = "v0.9.5"
$Version = "0.9.5"

New-Item -ItemType Directory -Force -Path $Staging | Out-Null
$exe = Get-ChildItem $Nsis -Filter "*0.9.5*x64-setup.exe" | Where-Object { $_.Name -notlike "*.sig" } | Select-Object -First 1
...
$notes = Get-Content (Join-Path $Root ".release-notes-0.9.5.md") -Raw -Encoding UTF8
...
if (-not $viewOk) {
  gh release create $Tag -R $Repo --title $Tag --notes-file (Join-Path $Root ".release-notes-0.9.5.md") --latest `
  ...
  gh release edit $Tag -R $Repo --notes-file (Join-Path $Root ".release-notes-0.9.5.md") --latest
}
