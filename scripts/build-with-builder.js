...
/**
 * Simplified build script for VeryAgent
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('VeryAgent.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running VeryAgent/Electron process. Attempting to close...');
        killWindowsProcesses(['VeryAgent.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running VeryAgent/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'VeryAgent.exe');
    ...
    console.log('⚠️  Windows local build failed after VeryAgent.exe was produced.');
    if (firstError) {
