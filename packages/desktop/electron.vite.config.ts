...
// Read the real VeryAgent version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@veryagent/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@veryagent/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@veryagent/web-host'] }),
        ...(isDevelopment
          ? [
              {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another VeryAgent clone is running),
      ...
      define: {
        ...
        __DEV_BACKEND_PORT__: JSON.stringify(25809),
        __DEV_FORCE_ELECTRON_MODE__: JSON.stringify(true),
        global: 'globalThis',
      },
      optimizeDeps: {
        exclude: ['electron'],
