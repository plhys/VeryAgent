 ...
 */
declare const __DEV_BACKEND_PORT__: number | undefined;
declare const __DEV_FORCE_ELECTRON_MODE__: boolean | undefined;

let _backendPortOverride = 0;

/** 开发模式下，外部可注入后端端口，绕过 Electron IPC 传值问题 */
export function setBackendPort(port: number): void {
  _backendPortOverride = port;
}

function getBackendPort(): number {
  // 1. Runtime injected by preload (Electron) — the authoritative source
  if (typeof window !== 'undefined') {
    const wp = (window as Window).__backendPort;
    // Dev safeguard: if preload injected 0, fall back to injected port
    if (wp && wp > 0) return wp;
  }
  // 2. Manual override
  if (_backendPortOverride > 0) return _backendPortOverride;
  // 3. Vite build-time constant (dev fallback)
  if (typeof __DEV_BACKEND_PORT__ !== 'undefined' && __DEV_BACKEND_PORT__ > 0) return __DEV_BACKEND_PORT__;
  // 4. Main process global
  const g = globalThis as typeof globalThis & { __backendPort?: number };
  if (g.__backendPort && g.__backendPort > 0) return g.__backendPort;
  // 5. Ultimate fallback
  return 13400;
}
...
function isWebUiBrowserMode(): boolean {
  if (typeof __DEV_FORCE_ELECTRON_MODE__ !== 'undefined' && __DEV_FORCE_ELECTRON_MODE__) return false;
  return (
