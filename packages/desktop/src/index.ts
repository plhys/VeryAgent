...
import { BackendLifecycleManager } from '@veryagent/web-host';
...
if (!gotTheLock) {
  console.warn('[VeryAgent] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[VeryAgent] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[VeryAgent] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[VeryAgent] runBackendMigrations completed');
    } catch (error) {
      console.error('[VeryAgent] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[VeryAgent] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  ...
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[VeryAgent] Ignoring AIONUI_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[VeryAgent] Ignoring unknown AIONUI_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[VeryAgent] Creating main window...');
  ...
  console.log(`[VeryAgent] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[VeryAgent] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[VeryAgent] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[VeryAgent] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[VeryAgent] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    ...
    console.log(`[VeryAgent] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[VeryAgent] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[VeryAgent] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[VeryAgent] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[VeryAgent] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[VeryAgent] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[VeryAgent] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[VeryAgent] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[VeryAgent] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[VeryAgent:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[VeryAgent] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[VeryAgent] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`VeryAgent --webui` or packaged `aionui-web` mode that
      ...
      // M6: Switch to @veryagent/web-host
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[VeryAgent] App initialization failed:', error);
    app.quit();
  });
...
app.on('will-quit', () => {
  console.log('[VeryAgent] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[VeryAgent] quit (exitCode=${exitCode})`);
});
