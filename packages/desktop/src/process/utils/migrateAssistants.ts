...
async function backendSupportsAssistantDefinitions(): Promise<boolean> {
  try {
    ...
    console.warn('[VeryAgent] Failed to probe unified assistant detail support:', error);
    return false;
  }
}
...
async function markAssistantsMigrationDone(configFile: ConfigFile): Promise<void> {
  ...
  try {
    ...
    console.warn('[VeryAgent] failed to persist assistants migration flag', err);
  }
}
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[VeryAgent] Skipped override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[VeryAgent] Failed to apply builtin override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(`[VeryAgent] Applied ${applied} builtin disabled-state override(s) (skipped ${skipped} retired id(s))`);
  } else {
    console.error(
      `[VeryAgent] Builtin override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[VeryAgent] Skipped agent_id override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[VeryAgent] Failed to apply agent_id override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(`[VeryAgent] Applied ${applied} builtin agent_id override(s) (skipped ${skipped} retired id(s))`);
  } else {
    console.error(
      `[VeryAgent] Builtin agent_id override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
...
async function fetchCurrentBuiltinAgentIds(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[VeryAgent] Failed to fetch current builtin agent_id map:', error);
    return new Map();
  }
}
...
async function fetchAgentIdByRuntimeKey(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[VeryAgent] Failed to fetch agent runtime identity map:', error);
    return new Map();
  }
}
...
async function uploadLegacyAssistantRules(legacyAssistantIds: Set<string>): Promise<number> {
  ...
  try {
    ...
    console.error('[VeryAgent] Failed to read legacy assistant rules dir:', error);
    return 1;
  }
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      failed += 1;
      console.error(
        `[VeryAgent] Failed to upload legacy rule for '${ruleEntries[i].id}' (${ruleEntries[i].locale}):`,
        r.reason
      );
      return;
    }
  ...
  if (failed === 0) {
    if (uploaded > 0 || skipped > 0) {
      console.log(`[VeryAgent] Legacy rule upload: ${uploaded} uploaded, ${skipped} skipped`);
    }
  } else {
    console.error(`[VeryAgent] Legacy rule upload partial: ${failed}/${ruleEntries.length} failed`);
  }
  return failed;
}
...
export async function migrateAssistantsToBackend(configFile: ConfigFile): Promise<boolean> {
  if (process.env.AIONUI_SKIP_ELECTRON_MIGRATION === '1') {
    console.log('[VeryAgent] Assistant migration skipped (env flag set)');
    return false;
  }
  ...
  if (userAssistants.length > 0) {
    try {
      ...
      if (result.failed !== 0) {
        console.error(`[VeryAgent] Assistant migration partial: ${result.failed} failed`, result.errors);
        return false;
      }
      if (result.imported > 0 || result.skipped > 0) {
        console.log(`[VeryAgent] migrated ${result.imported} assistants (skipped ${result.skipped})`);
      }
    } catch (error) {
      console.error('[VeryAgent] Assistant migration failed:', error);
      return false;
    }
  }
