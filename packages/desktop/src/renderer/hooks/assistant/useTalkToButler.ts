...
/** Backend manifest id of the built-in VeryAgent Butler assistant. */
const BUTLER_ASSISTANT_ID = 'veryagent-assistant';
 ...
 * select the VeryAgent Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the VeryAgent Butler for you' })
            );
          }
        }
      } catch (error) {
